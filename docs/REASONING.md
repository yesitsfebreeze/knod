...
4. **One executable, many strands.** A single knod binary handles everything. Each strand is a graph file on disk. Cross-store queries spawn subprocesses of the same binary, each loading one graph. No distribution needed — subprocess fan-out handles parallelism on one machine.

...
let graphCenter=[0,0,0], orbitCenter=[0,0,0], orbitTarget=[0,0,0], graphRadius=POLY_R, minDist=0.7, maxDist=6, resetDist=3;
...
function updateCameraBounds() {
  ...
  const asp=Math.max(W/Math.max(H, 1), 0.1);
  const halfW=sx*0.5*FRAME_PAD;
  const halfH=sy*0.5*FRAME_PAD;
  const halfD=sz*0.5;
  const tanHalf=Math.tan(FOV/2);
  const fitX=halfW/(tanHalf*asp);
  const fitY=halfH/tanHalf;
  const fitZ=halfD + maxNodeR*6;
  const fitDist=Math.max(fitX, fitY, fitZ, 0.8);

  graphRadius=Math.max(0.6, Math.max(halfW, halfH, halfD));
  minDist=Math.max(maxNodeR*2.0, fitDist*0.08);
  maxDist=Math.max(minDist*1.4, fitDist*MAX_VIEW);
...
function layout(data) {
  ...
  specs.forEach((s,i) => {
    ...
    const nd={key:s.key, pos:[...tagPos[s.label]], r:0.12, color:col,
  ...
  for (const [store,list] of Object.entries(bk)) {
    ...
    for (const t of list) {
      ...
      const nd={key:t.key, pos:[radial*Math.cos(theta), yJ, radial*Math.sin(theta)],
...
function makePipes(dev, fmt) {
  ...
  const ptP=dev.createRenderPipeline({layout:pl,
    vertex:{module:dev.createShaderModule({code:LNS}), entryPoint:"vs",
      buffers:[{arrayStride:28, attributes:[
        {shaderLocation:0,offset:0,format:"float32x3"},
        {shaderLocation:1,offset:12,format:"float32x4"},
      ]}]},
    fragment:{module:dev.createShaderModule({code:LNS}), entryPoint:"fs",
      targets:[{format:fmt, blend}]},
    primitive:{topology:"line-list"},
    depthStencil:{depthWriteEnabled:false, depthCompare:"less", format:"depth24plus"},
  });

  const lnP=dev.createRenderPipeline({layout:pl,
    vertex:{module:dev.createShaderModule({code:LNS}), entryPoint:"vs",
      buffers:[{arrayStride:28, attributes:[
        {shaderLocation:0,offset:0,format:"float32x3"},
        {shaderLocation:1,offset:12,format:"float32x4"},

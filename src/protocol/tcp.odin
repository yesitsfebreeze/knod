...
// _Conn_Job holds the per-connection state passed to the goroutine.
@(private)
_Conn_Job :: struct {
...
// tcp_poll drains every pending incoming connection from the OS accept queue,
// spawning a goroutine for each one so that reading/dispatching one connection
// never blocks acceptance of the next.
...
_conn_goroutine :: proc(t: ^thread.Thread) {
	...
	// Peek at the first byte to decide protocol:
	//   >= 0x20 (printable ASCII) → legacy single-shot connection
	//   < 0x20                    → framed persistent session (first 4 bytes = length)
	peek: [4]u8
	...
	if peek[0] >= 0x20 {
		// Legacy: read the rest until EOF, dispatch once, close.
		...
		// Framed session: we have 1 byte of the 4-byte length field — read the rest.
...
// _conn_session runs a persistent framed-message loop on a single socket.
// Frame format (both directions): [uint32 big-endian length][payload bytes]
// `first_len_buf` contains the 4 bytes of the first message's length field.
@(private)
_conn_session :: proc(sock: net.TCP_Socket, h: ^Handler, first_len_buf: []u8) {
	...
	for {
		...
		if msg_len == 0 {
			// Zero-length = keepalive ping — echo zero back and wait for next message.
			reply_hdr: [4]u8
		...
		reply := _dispatch_framed(h, string(body))
		defer delete(reply)
...
// _dispatch_framed handles one message in a persistent session.
// Returns a heap-allocated reply string (caller must delete).
@(private)
_dispatch_framed :: proc(h: ^Handler, text: string) -> string {
	ASK_PREFIX             :: "ASK:"
	STATUS_CMD             :: "STATUS"
...
// _dispatch_once handles a legacy single-shot connection (one message, then close).
@(private)
...
// _recv_exact reads exactly len(buf) bytes. Returns (_, true) on error/EOF.
@(private)

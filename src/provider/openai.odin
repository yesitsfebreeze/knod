...
openai_post :: proc(state: ^OpenAI_State, endpoint: string, payload: any) -> (string, bool) {
	url := fmt.tprintf("%s%s", state.base_url, endpoint)
	...
	http.headers_set_unsafe(&req.headers, "authorization", fmt.tprintf("Bearer %s", state.api_key))

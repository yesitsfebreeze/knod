...
After ingesting all articles the script sends SUBSCRIBE and blocks, receiving
server-pushed status frames until the queue is fully drained and the GNN step
count has stabilised.  No polling loop required.
...
def get_status(session: KnodSession) -> dict:
	try:
		return parse_status(session.send("STATUS"))
	except Exception:
		return {}


def wait_for_queue_drain(session: KnodSession) -> dict:
	"""Block until the ingest queue is empty and no goroutines are in-flight.

	Sends SUBSCRIBE once; the server pushes a status frame after every
	article goroutine completes (which includes GNN training).  No polling —
	we block on recv_frame() until queued=0 and in_flight=0.

	Because the server pushes *after* decrementing in_flight (which happens
	after GNN training), a push with in_flight=0 means all work is done.
	The old polling stable-for window is no longer needed.
	"""
	print("  Waiting for queue to drain...", flush=True)

	# Subscribe — server replies "subscribed", then starts pushing events.
	resp = session.send("SUBSCRIBE")
	...
	# Unsubscribe so the socket is clean for reuse.
	try:
		session.send("UNSUBSCRIBE")
	except Exception:
		pass

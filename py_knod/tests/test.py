...
def main():
	...
	parser.add_argument("--keep", action="store_true", help="don't wipe graph data before running")
	parser.add_argument(
		"--passes",
		type=int,
		default=3,
	...
	try:
		...
		if strand_step < GOOD_GNN_STEPS:
			print(f"\n  WARNING: strand_step={strand_step} < {GOOD_GNN_STEPS} (try --passes={passes + 2} for more training)")
		else:
